...
*   这个正态分布的**宽度（标准差）** 称为**标准误（Standard Error, $\sigma_x$）**，计算公式为：
...
应用到我们的抽样分布上，就意味着：
> 在所有可能抽出的样本中，有大约 **95%** 的样本均值 $\bar{x}$ 会落在区间 $[\mu - 1.96 \times \sigma_x,  \mu + 1.96 \times \sigma_x]$ 之内。

用不等式表示就是：
$$\mu - 1.96 \times \sigma_x \leq \bar{X} \leq \mu + 1.96 \times \sigma_x$$
...
**等价于**新不等式：
$$\bar{X} - 1.96 \times \sigma_x \leq \mu \leq \bar{X} + 1.96 \times \sigma_x$$

**新不等式的含义发生了根本变化：**
> 对于任何一次抽样得到的 $\bar{x}$，我们构造一个区间 $[\bar{x} - 1.96 \times \sigma_x,  \bar{x} + 1.96 \times \sigma_x]$。那么，在无数次重复这个构造过程后，有 **95%** 的区间会把真正的 $\mu$ 包含在内。
...
2.  **区间宽度由什么决定**：
    ...
    *   **样本大小 ($n$)**：样本量 $n$ 越大，标准误 $\sigma_x = \frac{\sigma}{\sqrt{n}}$ 越小，区间就越窄，估计越精确。
    *   **数据波动 ($\sigma$)**：总体数据本身越分散（$\sigma$ 越大），区间就越宽。

3.  **实际操作**：我们通常不知道总体标准差 $\sigma$，所以会用样本标准差 $s$ 来估计它，公式变为 $\sigma_x = \frac{s}{\sqrt{n}}$。当样本量较小时，会使用 **t分布** 而不是正态分布来寻找合适的乘数（例如从 t 表中查 $t^{*}$ 值来代替 1.96），但其核心思想完全一样。
